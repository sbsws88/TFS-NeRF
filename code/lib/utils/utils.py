...
def weighted_sampling(data, img_size, num_sample, bbox_ratio=0.9):
    ...
    mask = data["object_mask"] + data["semantic_map"]   ##### also consider object mask
    where = np.asarray(np.where(mask))
    if where.shape[1] == 0:
        bbox_min = np.asarray([1020, 730])
        bbox_max = np.asarray([1162, 866])
    else:
    ...
    x1 = [ int(x) for x in indices[:,0] ]
    y1 = [ int(x) for x in indices[:,1] ]

    ind_inside_human = np.where(mask[x1[:], y1[:]]>0)[0]   
    ...
    for key, val in data.items():
        ...
        output[key] = new_val
    return output, index_outside, ind_inside_human, indices


def obj_aware_weighted_sampling(data, img_size, num_sample, bbox_ratio=0.9):
    """
    More sampling within the bounding box
    """
    
    # calculate bounding box
    mask = data["semantic_map"]   ##### also consider object mask
    
    h_mask = np.zeros(mask.shape)
    h_mask[mask==1] = 1
    dial_kernel = np.ones((20, 20),np.uint8)
    h_mask = cv2.dilate(h_mask, dial_kernel)

    #### sample from human. first dilate the human mask to get some outside points
    where = np.asarray(np.where(h_mask==1))
    if where.shape[1] == 0:
        bbox_min = np.asarray([1020, 730])
        bbox_max = np.asarray([1162, 866])
    else:
    ...
    num_sample_bbox_h = int(0.6 * num_sample * bbox_ratio)
    samples_bbox_h = np.random.rand(num_sample_bbox_h, 2)
    samples_bbox_h = samples_bbox_h * (bbox_max - bbox_min) + bbox_min

    #### sample from obj. first dilate the obj mask to get some outside points
    o_mask = np.zeros(mask.shape)
    o_mask[mask==2] = 1
    dial_kernel = np.ones((20, 20),np.uint8)
    o_mask = cv2.dilate(o_mask, dial_kernel)    
    where = np.asarray(np.where(o_mask==1))
    if where.shape[1] == 0:
        bbox_min = np.asarray([1020, 730])
        bbox_max = np.asarray([1162, 866])
    else:
    ...
    num_sample_bbox_o = int(0.4 * num_sample * bbox_ratio)
    samples_bbox_o = np.random.rand(num_sample_bbox_o, 2)
    samples_bbox_o = samples_bbox_o * (bbox_max - bbox_min) + bbox_min    

    num_sample_uniform = num_sample - num_sample_bbox_h - num_sample_bbox_o
    samples_uniform = np.random.rand(num_sample_uniform, 2)
    samples_uniform *= (img_size[0] - 1, img_size[1] - 1)
    ...
    bbox_min = where.min(axis=1)
    bbox_max = where.max(axis=1)     
    index_outside = get_index_outside_of_bbox(samples_uniform, bbox_min, bbox_max) + num_sample_bbox_h + num_sample_bbox_o

    

    indices = np.concatenate([samples_bbox_h, samples_bbox_o, samples_uniform], axis=0)
    x1 = [ int(x) for x in indices[:,0] ]
    y1 = [ int(x) for x in indices[:,1] ]

    ind_inside_human_and_obj = np.where(mask[x1[:], y1[:]]>0)[0]

    '''plt.imshow(mask)
    plt.scatter(indices[ind_inside_human_and_obj,1], indices[ind_inside_human_and_obj,0], color='g')
    plt.scatter(indices[index_outside,1], indices[index_outside,0], color='b')
    plt.savefig('point_sample_now.png')
    exit()'''     

    output = {}
    for key, val in data.items():
        if key == 'semantic_map':     
            output[key] = val[x1[:], y1[:]]
        else:
            if len(val.shape) == 3:
                new_val = np.stack([
                    bilinear_interpolation(indices[:, 0], indices[:, 1], val[:, :, i])
            ...
            new_val = new_val.reshape(-1, *val.shape[2:])
            output[key] = new_val
    return output, index_outside, ind_inside_human_and_obj, indices
