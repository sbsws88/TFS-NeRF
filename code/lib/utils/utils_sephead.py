...
def obj_aware_weighted_sampling(data, img_size, num_sample, bbox_ratio=0.9):
    ...
    '''plt.imshow(mask)
    plt.scatter(indices[:samples_bbox_h.shape[0],1], indices[:samples_bbox_h.shape[0],0], color='g')
    ...
    for key, val in data.items():
        if key == 'semantic_map':     
            ...
            new_val = new_val.reshape(-1, *val.shape[2:])
            output[key] = new_val
    return output, index_outside, ind_inside_human_and_obj, indices, samples_bbox_h.shape[0]
