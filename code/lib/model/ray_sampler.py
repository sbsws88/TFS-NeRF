...
class ErrorBoundSampler(RaySampler):
    ...
    def get_sem_label_distonehot(self, pts, control_points, sigma=0.1, threshold=0.05):

        #### get distance of points from nearest joints in human
        human_skel = control_points[:,:24,:]
        human_dist, _, _ = ops.knn_points(pts.unsqueeze(0), human_skel.reshape(1,24,3),
                                                                    K=1, return_nn=True)  

        #### get distance of points from nearest joints in object
        obj_skel = control_points[:,24:,:]
        obj_dist, _, _ = ops.knn_points(pts.unsqueeze(0), obj_skel.reshape(1,24,3),
                                                                    K=1, return_nn=True)  

        sem_indctr = torch.cat([human_dist, obj_dist], -1)

        min_ind = torch.argmin(sem_indctr, -1) + 1
        min_val = torch.min(sem_indctr, -1).values

        bg_ind = min_val>threshold
        min_ind[bg_ind] = 0   

        return min_ind               

    def get_z_vals(self, ray_dirs, cam_code, epoch, cam_loc, model, cond, tfs, n_sample_h, eval_mode, jnts):
        ...
        if epoch > 1000:
            ...
            while not_converge and total_iters < self.max_total_iters:
                ...
                points_h = points[:n_sample_h,:,:].reshape(-1,3)
                points_o = points[n_sample_h:,:,:].reshape(-1,3)
                points_flat = points.reshape(-1, 3)

                # ##### semantic_label based on distance
                sem_label_flat = self.get_sem_label(points_flat, jnts)
                sem_label_flat_h = sem_label_flat[:,:points_h.shape[0],:].reshape(-1, 3)                                
                sem_label_flat_o = sem_label_flat[:,points_h.shape[0]:,:].reshape(-1, 3)
                sem_label_flat = sem_label_flat.reshape(-1, 3) 

                # Calculating the SDF only for the new sampled points
                model.implicit_network.eval()
                with torch.no_grad():
                    samples_sdf, _ = model.get_sdf_value(points_h.reshape(-1, 3), points_o.reshape(-1, 3), sem_label_flat_h, sem_label_flat_o, cam_code, cond, tfs, jnts, eval_mode=eval_mode)
